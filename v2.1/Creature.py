...
class Creature:
    ...
    def __init__(self, polygon, brain=None, ev=False, outp_size=5):
        ...
        self.eaten = 0
        self.health = 100
        self.inp_size = 3*4 + 3 + 1 + self.pace_bits
        self.outp_size = outp_size
        if brain == None:
            self.brain = br.Brain((self.inp_size, self.outp_size))
        else:
            self.brain = br.Brain((self.inp_size, self.outp_size), brain.matrix, ev)
                    
    def get(self):
        self.inp = []
        self.inp += self.polygon.up_not_near_transcript(self.coord, self.r)
        #self.inp += self.polygon.simple_transcript(self.coord, self.r)
        self.inp += self.polygon.near_transcript(self.coord)
    ...
    def update(self):
        if self.alive == False:
            self.polygon.creatures.append(self)
        self.eaten = 0
        self.lifetime = 0
        self.health = 100
        self.alive = True
...
class Rabbit(Creature):
    ...
    def born_loc(self, y, x):
        #return  self.polygon.coord_h(y, x) > 80
        return True or (self.polygon.coord_b(y, x) == 2 and self.polygon.coord_h(y, x) > 80)
