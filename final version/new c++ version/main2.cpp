...
double k_step = 0.004;
tuple <double, double, double> field[500][500]; // поле, определ€ющее разность давлений
    ...
    void pict(vector <vector <pair <int, double>>> &ans) {
        ...
        for (double i_p = 0; i_p < p_size; i_p++) {
            ...
            for (double j_p = 0; j_p < p_size; j_p++) {
                ...
                double ang2 = a_size / 2 - j * a_size / p_size + scal / sqrt(2); // измен€ющий фокус
